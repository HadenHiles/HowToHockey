...
<!-- Carousel -->
    <div class="home_carousel loading">    
        ...
        <div class="re_carousel" <?php if (rehub_option('rehub_homecarousel_label')=='1'): ?>style="margin-top:2px"<?php endif;?>>
                ...
                <div class="carousel-block">
                    <?php $result_cat = array();
                        ...
                        if (rehub_option('rehub_homecarousel_ed') == '1') {
                            $home_carousel = new WP_Query(array('posts_per_page' => -1, 'post__not_in' => $exclude_posts, 'meta_key' => 'is_editor_choice', 'meta_value' => '1', 'ignore_sticky_posts' => 1 ));
                        }
                        elseif ($tag_carousel !=''){
                            $home_carousel = new WP_Query(array('posts_per_page' => -1, 'post__not_in' => $exclude_posts, 'tag' => $tag_carousel, 'ignore_sticky_posts' => 1 ));
                        }
                        else {
                            $home_carousel = new WP_Query(array('posts_per_page' => -1, 'post__not_in' => $exclude_posts, 'ignore_sticky_posts' => 1 ));
                        }    
                    ...
                    <div class="preview tabcat-<?php $category = get_the_category($post->ID); $first_cat = $category[0]->term_id; $result_cat[] = $first_cat; echo $first_cat; ?>">
                        <figure>
                            ...
                            <a href="<?php the_permalink();?>">
                                <?php $img = get_post_thumb(); $params = array( 'width' => 252, 'height' => 160, 'crop' => true); ?>
            ...
            <?php 
                ...
                foreach($category_ids as $cat_id) { ?>      
                    ...
                    <?php if (!empty($cat_data['cat_color'])) :?>
                        .re_carousel .tabcat-<?php echo $cat_id ;?> .text-overlay-oncarousel{border-bottom: 6px solid <?php echo $cat_data['cat_color'];?> !important; color:#fff !important; text-decoration: none;}
                    <?php else :?><?php endif;?>
                <?php }
                echo '</style>';
            ?>             
