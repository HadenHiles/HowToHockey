<?php 
    $type = $enable_pagination = $cat = $tag = $ids = $orderby = $order = $show = $columns = $show_coupons_only = '';
    $data_source = 'cat';
    $order = 'DESC';
...
<?php while ( $wp_query->have_posts() ) : $wp_query->the_post();  global $product;  ?>
    ...
    <article class="woocommerce yith_float_btns column_grid<?php if (($i % 3) == '0') :?> last-col<?php endif ?><?php if (($i % 3) == '1') :?> first-col<?php endif ?><?php echo $coupon_enabled_style ;?>">
    <?php elseif ($columns == '4_col') :?>
    <article class="woocommerce yith_float_btns col_4_grid column_grid<?php if (($i % 4) == '0') :?> last-col<?php endif ?><?php if (($i % 4) == '1') :?> first-col<?php endif ?><?php echo $coupon_enabled_style ;?>">
    <?php endif ;?> 
